...
phi_mn = wrapTo180(phi_0 + phi_rand);

phi_quant = abs(180 * floor((phi_mn)/180 + 0.5)) ; % 1-bit quantization
phi_total = phi_quant + phi_rand;
...
for x = 1:M
    for y = 1:N
        AF = AF + exp(-1j*phi_total(x,y)*pi/180) * exp(-1j*k0*((x-M/2-1)*dx*u + (y-N/2-1)*dy*v));
    end
end
...
% Plot
figure;
subplot(2,2,1); imagesc(x,y,phi_rand); title('(a) \phi^{rand}_{MN}'); 
axis square; colorbar; colormap jet; 
subplot(2,2,2); imagesc(phi_mn); title('(b) excitation phase 32 × 32 \phi_{mn}'); 
